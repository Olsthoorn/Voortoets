...
class Grid:
    ...
    are meant and ncbd wherever the confining beds are meant. Only when there
    ...
    @property
    def nodlay(self):
        """Return number of cells in a layer."""
    ...
    def LRC(self, Iglob, astuples=None, aslist=None):
        ...
        return self.lrc_from_iglob(Iglob, astuples=astuples, aslist=aslist)
    ...
    def to_struct(self, slice, arraydict=None):
        """Return struct array specified by dtype and arraydict.
        
        Parameters
        ----------
        slice: slice
            selection from self.NOD to get the cell indices
        arraydict: dict with field-item pairs like {'h': H, 'C': c}
            the arrays that are converted into a struct array
        """
        fields= [('I', '<i4')]
        for fld, array in arraydict.items():
            fields.append((fld, array.dtype))
        dtype =np.dtype(fields)
        
        idx = self.NOD[slice].flatten()
        sarray = np.zeros(len(idx), dtype=dtype)
        sarray['I'] = idx
        for fld, array in arraydict.items():
            sarray[fld] = array[idx]
        return sarray
    
    def check_struct(self, sarray_dict, dtype=None):
        """Verify that sarray_dict is dict of structarray with at least field ('I', '<i4') and keys are it.

        Parameters
        ----------
        sarray_dict: dict with keys [0, ...] of stuct_arrays with at least index field ('i', '<i4').
            the arrays with their stress period as key.
        dtype: dtype | optional
            the expected dtype or None, then we check only the presence of field ('I', '<i4')
        """
        assert isinstance(sarray_dict, dict), 'input not dict of structarrays'
        idx = list(sarray_dict.keys())
        assert np.all([isinstance(i, int) for i in idx]), "all sarray_dict keys must be integers."
        assert idx[0] == 0, "first key in sarray_dict must  be 0"
        assert np.all(np.diff(np.array(idx)) > 0), "indices in sarray_dict must be increasing."
        for it in idx:
            if dtype:
                assert sarray_dict[it].dtype == dtype, f"sarray_dict[{it}] must have dtype {dtype}"
            else:
                'I' in sarray_dict[it].dtype.fields
                np.all(sarray_dict[it]['I'] < self.nod), f"all indices in field 'I' must be < {self.nod}"
        return sarray_dict

    
    
    def fr_struct(self, sarray=None):
        """Return list of arrays from sarray, based on its dtype which must include index field 'I'.
        
        parameters
        ----------
        sarray: struct array with dtype including at least one field 'I' used for indexing
            the struct array
        """
        flds = sarray.dtype.fields()
        fmts = sarray.dtype.formats()
        assert 'I' in flds, f"'I' must be field in dtype sarrray, dtype is {sarray.dtype}"
        idx = sarray['I']
        arrays = []
        for fld, fmt in zip(flds, fmts):
            if fld == 'I':
                continue
            A = self.const(0., dtype=fmt)
            A.ravel()[idx] = sarray[fld]
            arrays.append(A)
        return arrays
    ...
    def GHB(self, cells, hds, cond):
        ...
        if np.isscalar(hds) or len(hds) == N:
            ...
            raise ValueError(f"hds in ghb must be of shape ({self.shape}) or of lenth {N} (corresponding to the boolean array cells)")
        if np.isscalar(cond) or len(cond) == N:
            ghb['C'] = cond
        elif np.all(cond.shape == self.shape):
            ghb['C'] = cond[cells]
        else:
            raise ValueError(f"cond in ghb must be of shape ({self.shape}) or of length {N} (corresponding to the boolean array cells)")
    ...
    def lines2HFB(self, lines, open=True, layers=0, cs=np.nan ):
        ...
        if not (len(lines) == len(layers) and len(lines) == len(cs)):
            raise ValueError(
              f'Nr of layers ({len(layers)}) or of c values ({len(cs)}) does not equal number of lines ({len(lines)}).')
    ...
    def interpxy(self, Z, points, iz=0, **kwargs):
        ...
        assert Z.shape[-2:] == self._shape[-2:],\
            f'Z must be 3D of shape (nz,{self._ny},{self._nx})'
